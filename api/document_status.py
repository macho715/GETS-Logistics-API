...
# ==================== Airtable API ====================
def get_airtable_headers() -> Optional[Dict[str, str]]:
    """Generate Airtable API headers"""
...
def fetch_table_records(table_name: str, filter_formula: str = None, max_records: int = 100) -> List[Dict]:
    """
...
def build_bottleneck_info(shipment_fields: Dict, bottleneck_code_record: Optional[Dict]) -> Dict:
    """
...
def build_action_info(
    shipment_fields: Dict,
    actions: List[Dict],
    bottleneck_code_record: Optional[Dict]
) -> Dict:
    """
...
def index():
    ...
    return jsonify({
        "message": "GETS Action API for ChatGPT - SpecPack v1.0",
        "status": "online",
        "version": "1.4.0",
        "dataSource": "Airtable (Real-time)" if AIRTABLE_API_TOKEN else "Not Connected",
...
def health_check():
    ...
    headers = get_airtable_headers()
    configured = headers is not None
    
    connected = False
    if configured:
        try:
            # Test connection with Shipments table
            test_records = fetch_table_records("shipments", max_records=1)
            connected = True
        except:
            connected = False
...
def get_approval_status(shpt_no: str):
    ...
    for approval in approvals:
        ...
        approval_list.append({
            ...
            "evidenceIds": fields.get("evidenceIds", "").split(",") if fields.get("evidenceIds") else []
        })
...
def get_status_summary():
    ...
    for doc_type in doc_types:
        completed = sum(
            ...
            and doc.get("fields", {}).get("status") in ["ISSUED", "RELEASED", "APPROVED"]
        )
        total = sum(
            1 for doc in documents
            if doc.get("fields", {}).get("docType") == doc_type
    ...
    return jsonify({
        ...
        **completion_rates,
        ...
        "topBottlenecks": [{"code": code, "count": count} for code, count in top_bottlenecks],
        "lastUpdated": now_dubai()
    })
