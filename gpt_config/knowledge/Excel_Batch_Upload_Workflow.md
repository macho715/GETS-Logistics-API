...
When user requests Excel upload with clear intent (e.g., "[SheetName] 업로드", "Excel Airtable 업로드", "진행"), execute immediately without asking multiple confirmation questions.
...
2. For each row:
   - Extract shptNo column value
   - Normalize shptNo: Convert to uppercase, handle variations
   - Search existing record:
     ...
     ```
   - If found: Extract recordId, prepare update payload, execute update
   - If not found: Skip row (log for summary)
...
2. For each row:
   - Prepare fields object with mapped values
   - Execute create (if Airtable supports batch create) or process individually
